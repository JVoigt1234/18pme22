...
void DatabaseController::generateDataset(QDateTime timestamp, double min, double max, int minStep, int count, bool sugarValue, bool randomSkip, QList<Measurement>& listOfMeasurements)
{
    qsrand(uint(QTime::currentTime().msec()));
    double f;
    double value, temp;
    ...
    {
        timestamp = timestamp.addMSecs(minStep*60*1000);
        f = qrand();
        if(randomSkip)
        {
            if( (int(f) % count) < int(count/4))
                timestamp = timestamp.addMSecs(minStep*60*1000);
        }
        value = double((f / RAND_MAX)) * (max - min) + min ;
        if(sugarValue == true)
        {
            listOfMeasurements.append(Measurement(timestamp, value));
            continue;
        }
        f = qrand();
        temp = double((f / RAND_MAX)) * ((value-20) - min) + min;
        listOfMeasurements.append(Measurement(timestamp,value, temp));
    }
}
