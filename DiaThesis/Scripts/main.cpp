...
int main(int argc, char *argv[])
{
    ...
    data = new DatabaseController("db.inftech.hs-mannheim.de"); //"1814116_PME2Development"
//***************************************************************************************
//    data->creatDatabase();
//    data->loadDateset();
...
//    data->generateDataset(QDateTime::currentDateTime(), 50, 150, 15, 10, true, false, testList);
//    for(int i = 0; i < testList.length(); i++)
//    {
//        qDebug() << testList[i].getValue();
//    }
//    testList.clear();
    data->generateDataset(QDateTime::currentDateTime(), 50, 150, 23, 100, false, true, testList);
    for(int i = 0; i < testList.length(); i++)
    {
        //qDebug() << testList[i].getValue();
        qDebug() << testList[i].getTimeStamp();
    }
    testList.clear();
    qDebug() << data->getFact();
//    int deleted = 0;
